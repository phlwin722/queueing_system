      ...
      <q-card class="q-pa-md">
        <q-form @submit.prevent="submitForm">
          ...
          <div v-if="form.auto_reset">
            ...
            <div v-if="form.reset_type === 'Weekly'" class="q-my-md">
            ...
            <div v-if="form.reset_type === 'Monthly'" class="q-my-md">
              <q-card flat bordered class="q-pa-md">
                <q-input
...
export default {
  setup() {
    ...
    const validateDate = () => {
      ...
      if (selectedDate > maxDate) {
        $notify(
          "negative",
          "error",
          "Invalid date. Maximum allowed date is 9999-12-31."
        );
        form.value.reset_date = "";
      }
    };
    ...
    const isFormInvalid = computed(() => {
      ...
      if (form.value.reset_type === "Daily" && !form.value.reset_time)
        return true;
      if (
        form.value.reset_type === "Weekly" &&
        (!form.value.reset_day || !form.value.reset_time)
      )
        return true;
      if (
        form.value.reset_type === "Monthly" &&
        (!form.value.reset_date || !form.value.reset_time)
      )
        return true;
      return false;
    });
