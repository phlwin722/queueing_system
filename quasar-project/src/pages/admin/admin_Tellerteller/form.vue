    ...
    <q-card class="relative-position">
      ...
      <q-card-section>
        <div class="row q-gutter-md">
          ...
          <div
            class="column items-center col-shrink justify-center q-gutter-md"
          >
...
export default defineComponent({
  ...
  setup(props) {
        ...
        try {
          ...
          if (typeof formData.value.type_ids_selected === "string") {
            try {
              ...
              formData.value.type_ids_selected = JSON.parse(
          ...
          formData.value.type_ids_selected =
            formData.value.type_ids_selected.map((id) => parseInt(id, 10));

          // Map each selected type ID to its corresponding category from categoriesList
          const selectedCategories = formData.value.type_ids_selected.map(
            (id) => {
    ...
    const handleSubmitForm = async () => {
      if (formData.value.role === "Teller") {
        try {
          ...
          if (
            formDataPassword.value.teller_newPassword !==
            formDataPassword.value.teller_retypepassword
          ) {
            $notify(
              "negative",
              "error",
          ...
          if (mode === "/update") {
            if (
              ...
              formData.value.teller_password =
                formDataPassword.value.teller_newPassword;
            }
          ...
          if (error.response.status === 422) {
            ...
            $notify(
              "negative",
              "error",
              "New password is required for this role change!"
            );
          } else if (error.response.status === 401) {
            $notify(
              "negative",
              "error",
              "Reuploading image is required for this role change!"
            );
          } else {
    ...
    watch(
      ...
      async (newVal, oldnewVal) => {
        if (newVal) {
          if (
            adminInformation.value == null &&
            formMode.value === "Edit" &&
            oldnewVal !== ""
          ) {
            formData.value.type_ids_selected = [];
