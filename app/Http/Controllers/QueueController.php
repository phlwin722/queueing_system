...
class QueueController extends Controller
{
    ...
    public function startWait(Request $request)
    {
        ...
        \DB::table('queues')->update(['waiting_customer' => $request->queue_number]);

        return response()->json(['message' => 'Customer is now being waited on.', 'waiting_customer' => $request->queue_number]);
    }
    ...
    public function getQueueList()
    {
        ...
        return response()->json([
            ...
            'current_serving' => Queue::where('status', 'serving')->first()?->queue_number ?? 'N/A',
    ...
    {
        Queue::where('queue_number', $request->queue_number)->update(['status' => 'cancelled']);
        return response()->json(['message' => 'Queue left successfully']);
    }

    public function getCQueueList()
    {
        return response()->json([
            'queue' => Queue::where('status', 'waiting')->orderBy('created_at')->get(),
    ...
    {
        Queue::where('status', 'serving')->update(['status' => 'served']);

        Queue::where('id', $request->id)->update(['status' => 'serving']);
    ...
    {
        Queue::where('id', $request->id)->update(['status' => 'cancelled']);
    ...
    public function finishCustomer(Request $request)
    {
        ...
        return response()->json(['message' => 'Customer marked as finished.']);
    }
