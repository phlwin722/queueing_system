        ...
        <div class="column items-center q-ma-lg">
          ...
          <div class="text-center q-mt-md">
            <div class="text-h6">
              ...
              <template v-else>
                {{
                  previewAdminImage?.Firstname +
                  " " +
                  previewAdminImage?.Lastname
                }}
                <div v-if="previewAdminImage?.branch_name">
                  <p class="text-gray q-mt-md">
                    {{ previewAdminImage?.branch_name }}
                  </p>
                  <p
                    class="text-gray"
                    style="margin-top: -30px; font-size: 15px; color: #1c5d99"
                  >
                    Branch
                  </p>
                </div>
            ...
            <q-form @submit="saveChanges">
              <div class="row q-col-gutter-md">
                <q-input
                  ...
                  :error-message="
                    formError.Username ? formError.Username[0] : ''
                  "
                ...
                <q-input
                  ...
                  :error-message="
                    formError.Firstname ? formError.Firstname[0] : ''
                  "
                  class="col-12 col-md-4"
                />
                ...
                <q-input
                  ...
                  :error-message="
                    formError.Lastname ? formError.Lastname[0] : ''
                  "
                  class="col-12 col-md-4"
                />
              </div>
    ...
    const UpdateAdmin = async () => {
      $dialog
        ...
        .onOk(async () => {
          try {
            $dialog.loading.show({
              message: 'Process in progress. Hang on...'
            })
            // Capitalize each string field individually
            adminInfo.value.Username = capitalizeWords(
              adminInfo.value.Username
            );
            adminInfo.value.Firstname = capitalizeWords(
              adminInfo.value.Firstname
            );
            adminInfo.value.Lastname = capitalizeWords(
              adminInfo.value.Lastname
            );

            const { data } = await $axios.post(
              "/admin/updateInformation",
              adminInfo.value
            );
            ...
            $dialog.loading.hide(); // Hide loading dialog  
          }
        })
        .onDismiss(() => {
        ...
        .onOk(async () => {
          ...
          try {
            ...
            if (
              ...
              adminPassword.value.confirmPassword
            ) {
              $notify(
                "negative",
                "error",
                "Passwords do not match. Please check your input data."
              );
              return;
            }
